...
export class Command<A extends any[] = any[], O extends {} = {}>{
    ...
    // 定义子指令
    subcommand<D extends string>(def: D,triggerEvent:TargetType|'all'): Command<Argv.ArgumentType<D>> {
    ...
    async execute(argv:Argv<A, O>):Promise<string|boolean|void>{
        ...
        let result
        try{
            result=await this.plugin.bail('before-command',argv)
        }catch {}
...
export namespace Command{
    ...
    export interface DefineOptions<D extends string=string>{
        def:D
        trigger:"group"|'private'|'all'
        auth?:number
        desc?:string|string[]
        alias?:string|string[]
        shortcuts?:DefineShortcut|DefineShortcut[]
        options?:DefineOption|DefineOption[]
        actions?:Callback|Callback[]
        checkers?:Callback|Callback[]
    }
}
