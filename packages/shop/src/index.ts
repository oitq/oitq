...
export default async function scan(config: ScanConfig) {
    ...
    async function search(offset: number) {
        const result = await config.request<SearchResult>(`/-/v1/search?text=oitq+plugin&size=250&offset=${offset}`)
    ...
    async function analyze(object: SearchObject) {
        ...
        const official = name.startsWith('@oitq/plugin-')
        const community = name.startsWith('oitq-plugin-')
