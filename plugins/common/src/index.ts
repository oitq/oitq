...
export function feedback(plugin: Plugin, {
    operators,
    timeout = 1000 * 60 * 60
}: { operators: number[], timeout?: number }) {
...
export function basic(plugin: Plugin, config: BasicConfig = {feedback: []}) {
    ...
    if (operators.length) plugin.plugin(feedback, {
        operators,
        timeout: noTimeout(config.feedback) ? plugin.app.config.delay.prompt : config.feedback['timeout']
    })
...
export function install(plugin: Plugin, config: Config) {
    ...
    plugin.command('common/segment/fake [user_id:qq] [message]', 'message')
        .option('multiple', '-m 制作多条')
        .desc('制作假的合并消息')
        .action(async ({session,bot, options}, user_id, message) => {
            if(!options.multiple && (!message || !user_id)) return 'message and user_id is required'
            ...
            let finished=true
            if(options.multiple)finished=false
            while (!finished){
                const {user_id,message,Continue}=await session.prompt([
                    {
                        type:'text',
                        message:'请输入消息内容',
                        name:'message'
